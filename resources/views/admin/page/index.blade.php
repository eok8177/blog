...
<div class="card">
  <div class="card-header bg-light d-sm-flex justify-content-between align-items-center">
    <h5 class="mb-0">{{ __('Static Pages') }}</h5>
      ...
      <table class="table table-hover align-middle">
        ...
        @foreach($pages as $page)
          <tr>
            ...
            <td>
              <a href="{{route('admin.page.edit', $page)}}" class="text-body">{{$page->slug}}</a>
            </td>

            <td>{{$page->title}}</td>
            <td>{{ str_cut($page->description, 80) }}</td>

            <td>{{$page->created_at->format('d.m.Y H:i')}}</td>

            <td class="text-start text-nowrap" width="1%">
              <a href="{{route('admin.page.edit', $page)}}" class="btn btn-sm">
                <i class="fa fa-pencil-square-o"></i>
              </a>

              <a href="{{route('admin.page.destroy',$page)}}" class="delete btn btn-sm text-danger ms-3">
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="addModalLabel">{{ __('Create New Page') }}</h5>
